...
int main(int argc, char** argv) {
	...
	if(verbose) cerr << "Reading SA from file " << sa_filename << endl;
	char filenm[1000];
	sprintf(filenm,"%s",sa_filename.c_str());
	FILE *fp = fopen(filenm,"r");
	if (!fp) { fprintf(stderr,"Could not open file, %s\n",filenm); exit(2); }
	BWT * b = read_BWT_bin(fp);
	fclose(fp);


	if(verbose) cerr << "Reading FM Index from file " << fmi_filename << endl;
	sprintf(filenm,"%s",fmi_filename.c_str());
	fp = fopen(filenm,"r");
	if (!fp) { fprintf(stderr,"Could not open file, %s\n",filenm); exit(2); }
	FMI * f = read_fmi(fp);
	fclose(fp);
	if(verbose) fprintf(stderr,"BWT of length %ld has been read with %d sequencs, alphabet=%s\n", b->len,b->nseq, b->alphabet); 
	...
	cerr << "Classifying reads using " << num_threads <<" threads..."<< endl;
	...
	if(verbose) cerr << getCurrentTime() << " Start classification" << endl;
	...
	if(verbose) cerr << getCurrentTime() << " Done classification" << endl;
